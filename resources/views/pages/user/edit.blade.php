  ...
  <div class="col-md-12">
    <nav aria-label="breadcrumb" class="float-right mt-1">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">{{ trans('Update User Role') }}</li>
      </ol>
    </nav>
    <h4 class="mb-1 mt-0">{{ trans('Update User Role') }}</h4>
  </div>
</div>
        ...
        <div class="row mg-2">
          <div class="col-12">
            <form action="{{ route('roles.update', $role) }}" method="post">
              ...
              <input type="text" value="{{ $role->label }}" name="label" class="form-control" id="name" required
                  placeholder="Enter name of role...">
              </div>

              <div class="form-group col-lg-8">
                <label for="sw-dots-userName">Description</label>
                <textarea name="description" rows="2" class="form-control" id="description" placeholder="Description..." value="{{ $role->description }}">{{ $role->description }}</textarea>
              </div>
