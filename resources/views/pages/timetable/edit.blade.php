  ...
  <div class="col-md-12">
    <nav aria-label="breadcrumb" class="float-right mt-1">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">{{ trans('Edit Timetable') }}</li>
      </ol>
    </nav>
    <h4 class="mb-1 mt-0">{{ trans('Edit Timetable') }}</h4>
  </div>
</div>
        ...
        <div class="row mg-2">
          <div class="col-12">
            <form action="{{ route('timetables.update', $timetable) }}" method="POST">
              ...
              <input type="text" name="title" class="form-control" value="{{ $timetable->title }}" id="sw-dots-userName" required
                  placeholder="Enter title for timetable...">
              </div>

              <div class="form-group col-lg-8">
                <label for="sw-dots-userName">Description</label>
                <textarea name="description" class="form-control" rows="2" id="example-textarea" required>{{ $timetable->description }}</textarea>
              </div>
